...
public class SqlQueryComparator {
	...
	public void compareStatical() throws Exception {
		...
		if (!userParsed.getMainKeyWord().equals(solutionParsed.getMainKeyWord())) {
			messages.add(new SemanticError(msgPrefix + System.getProperty("COMPARATOR.SEMANTIC_ERROR"),
	...
	public boolean compareDynamical() {
		...
		if (visitorBased) {
			messages.add(new ResultError(System.getProperty("COMPARATOR.DYN_RESULT"), System
					.getProperty("COMPARATOR.DYN_RESULT.FAIL"), false));
			return false;
		}
		...
		try {
			...
			if (userQuery.getError() == null && solutionQuery.getError() == null) {
				...
				if(tmp.size() == 0){
					messages.add(new ResultError(System.getProperty("COMPARATOR.DYN_RESULT"), System
							.getProperty("COMPARATOR.DYN_RESULT.SUC"), true));
				} else {
					messages.add(new ResultError(System.getProperty("COMPARATOR.DYN_RESULT"), System
							.getProperty("COMPARATOR.DYN_RESULT.FAIL"), false));
				}
	...
	private String getStringForKeyWord(int num, String keyWord, String customText) {
		...
		if (num == 0) {
			return fillPropertyString(System.getProperty("COMPARATOR.KEYWORD_MISSING"), new String[] { keyWord });
		} else if (num == 3) {
			return fillPropertyString(System.getProperty("COMPARATOR.TYPO_ERROR"), new String[] { customText });
		} else if (keyWord.equals("FROM")) {
			if (num == 1)
				return fillPropertyString(System.getProperty("COMPARATOR.TABLE_MISSING"), new String[] { customText });
			else if (num == 2)
				return fillPropertyString(System.getProperty("COMPARATOR.TABLE_SURPLUS"), new String[] { customText });
		} else if (keyWord.equals("COLUMNS")) {
			if (num == 1)
				return fillPropertyString(System.getProperty("COMPARATOR.COLUMN_MISSING"), new String[] { customText });
			else if (num == 2)
				return fillPropertyString(System.getProperty("COMPARATOR.COLUMN_SURPLUS"), new String[] { customText });
		} else if (keyWord.equals("TARGETCOLUMNS")) {
			if (num == 1)
				return fillPropertyString(System.getProperty("COMPARATOR.TAR_COL_MISSING"), new String[] { customText });
			else if (num == 2)
				return fillPropertyString(System.getProperty("COMPARATOR.TAR_COL_SURPLUS"), new String[] { customText });
		} else if (keyWord.equals("VALUES")) {
			if (num == 1)
				return fillPropertyString(System.getProperty("COMPARATOR.VAL_COL_MISSING"), new String[] { customText });
			else if (num == 2)
				return fillPropertyString(System.getProperty("COMPARATOR.VAL_COL_SURPLUS"), new String[] { customText });
		} else if (keyWord.equals("ORDERBY")) {
			if (num == 1)
				return fillPropertyString(System.getProperty("COMPARATOR.ORDERBY_MISSING"), new String[] { customText });
			else if (num == 2)
				return fillPropertyString(System.getProperty("COMPARATOR.ORDERBY_SURPLUS"), new String[] { customText });
		} else if (keyWord.equals("GROUPBY")) {
			if (num == 1)
				return fillPropertyString(System.getProperty("COMPARATOR.GROUPBY_MISSING"), new String[] { customText });
			else if (num == 2)
				return fillPropertyString(System.getProperty("COMPARATOR.GROUPBY_SURPLUS"), new String[] { customText });
		} else if (keyWord.equals("WHERE") || keyWord.equals("HAVING")) {
			if (num == 1)
				return fillPropertyString(System.getProperty("COMPARATOR.COND_WRONG"), new String[] { keyWord });
			else if (num == 2)
				return fillPropertyString(System.getProperty("COMPARATOR.COND_SURPLUS"), new String[] { keyWord });
		} else if (keyWord.equals("SELECT") || keyWord.equals("INSERT") || keyWord.equals("UPDATE")
				|| keyWord.equals("DELETE")) {
			if (num == 1)
				return fillPropertyString(System.getProperty("COMPARATOR.KEYWORD_WRONG"), new String[] { keyWord,
						customText });
		} else if (keyWord.equals("LIMIT")) {
			if (num == 1)
				return System.getProperty("COMPARATOR.LIM_OFFSET_WRONG");
			else if (num == 2)
				return System.getProperty("COMPARATOR.LIM_NUMBER_WRONG");
		}
