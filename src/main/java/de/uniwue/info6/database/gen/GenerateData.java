...
public class GenerateData {
	...
	private static final String createScriptFileWithExample = createScriptDir + "backup_latest_with_data.sql";
	private static final String oldExercises = "0" + File.separator + "miscellaneous" + File.separator
			+ "example_scenario.md";
	...
	private String getRightsScript(String dbHost, String dbUser, String dbName, String dbPass) {
		String addRightsScripts = StringTools.stripHtmlTags(System.getProperty("EDIT_SC.DESCRIPTION"));
	...
	public void resetDB(boolean insertExampleScenario, boolean dropAndCreateDB) {
		...
		try {
			...
			dbName = System.getProperty("MAIN_DBNAME");

			if (insertExampleScenario) {
				dummy = new Scenario(DUMMY, new Date(), null, null, "", createScriptFileWithExample, null, dbHost,
						dbUser, dbPass, dbPort, dbName);
			} else {
				dummy = new Scenario(DUMMY, new Date(), null, null, "", createScriptFile, null, dbHost, dbUser, dbPass,
						dbPort, dbName);
			}
					...
					if (scenarios != null && !scenarios.isEmpty()) {
						for (Scenario sc : scenarios) {
							if (sc != null) {
								...
								dbHost = System.getProperty("EXAMPLE_SCENARIO_DBHOST");
								dbUser = System.getProperty("EXAMPLE_SCENARIO_DBUSER");
								dbPass = System.getProperty("EXAMPLE_SCENARIO_DBPASS");
								dbPort = System.getProperty("EXAMPLE_SCENARIO_DBPORT");
								dbName = System.getProperty("EXAMPLE_SCENARIO_DBNAME");
								...
								if (!hasUserData && allowCreation != null && StringTools.parseBoolean(allowCreation)) {
	...
	public void insertExampleScenario(boolean insertUserData) {
		...
		String description = "<h1>Amazon Warenhaus</h1>" + "<p>Sie fangen einen neuen Job im Amazon Warenhaus "
				...
				+ " Ihnen in den folgenden Kapiteln ein" + "n&#228;herer Einblick gew&#228;hrt werden soll. "
				...
				+ " Informationen aus verschiedenen Tabellen " + "selektieren und aufbereiten k&#246;nnen.</p>";

		// example of an exercise-database
		String name = "SWT2014", dbHost = System.getProperty("EXAMPLE_SCENARIO_DBHOST"), dbUser = System
				.getProperty("EXAMPLE_SCENARIO_DBUSER"), dbPass = System.getProperty("EXAMPLE_SCENARIO_DBPASS"), dbPort = System
				.getProperty("EXAMPLE_SCENARIO_DBPORT"), dbName = System.getProperty("EXAMPLE_SCENARIO_DBNAME");
		...
		scenario = new Scenario(name, new Date(), null, null, description, createScriptFile, "er-diagram.png", dbHost,
				dbUser, dbPass, dbPort, dbName);
		...
		exgroup = new ExerciseGroup(scenario, null, "Select-Aufgaben", null, new Date(), null, null, false, null, null);
		...
		exercise = new Exercise(exgroup, null, question, "Select", (byte) 2, new Date(), null, null, null, null, null);
		...
		if (insertUserData) {
			// public UserExTag(User user, Exercise exercise, Date lastModified, String tag) {
		...
		exercise = new Exercise(exgroup, null, question, "Select", (byte) 1, new Date(), null, null, null, null, null);
		exerciseDao.insertNewInstance(exercise);
		...
		//	user_entry = new UserEntry(user, exercise, "Select * from books", new Date());
		...
		ExerciseGroup exgroupUpdate = new ExerciseGroup(scenario, null, "Update-Aufgaben", null, new Date(), null,
				null, false, null, null);
		exgroupDao.insertNewInstance(exgroupUpdate);
		ExerciseGroup exgroupInsert = new ExerciseGroup(scenario, null, "Insert-Aufgaben", null, new Date(), null,
				null, false, null, null);
		exgroupDao.insertNewInstance(exgroupInsert);
		ExerciseGroup exgroupDelete = new ExerciseGroup(scenario, null, "Delete-Aufgaben", null, new Date(), null,
				null, false, null, null);
		exgroupDao.insertNewInstance(exgroupDelete);
		ExerciseGroup exgroupRated = new ExerciseGroup(scenario, null, "Übungsblatt 4 - Aufgabe 1", null, new Date(),
		...
		try {
			...
			while (scan.hasNextLine()) {
				...
				if (counter++ % 7 == 0) {
					exercise = new Exercise(exgroupRated, null, q, type, Byte.parseByte(c), new Date(), null, null,
							null, null, null);
					exerciseDao.insertNewInstance(exercise);
					solution = new SolutionQuery(exercise, s);
					solutionDao.insertNewInstance(solution);
				} else if (type.toLowerCase().equals("select")) {
					exercise = new Exercise(exgroup, null, q, type, Byte.parseByte(c), new Date(), null, null, null,
							null, null);
					exerciseDao.insertNewInstance(exercise);
					solution = new SolutionQuery(exercise, s);
					solutionDao.insertNewInstance(solution);
				} else if (type.toLowerCase().equals("update")) {
					exercise = new Exercise(exgroupUpdate, null, q, type, Byte.parseByte(c), new Date(), null, null,
							null, null, null);
					exerciseDao.insertNewInstance(exercise);
					solution = new SolutionQuery(exercise, s);
					solutionDao.insertNewInstance(solution);
				} else if (type.toLowerCase().equals("delete")) {
					exercise = new Exercise(exgroupDelete, null, q, type, Byte.parseByte(c), new Date(), null, null,
							null, null, null);
					exerciseDao.insertNewInstance(exercise);
					solution = new SolutionQuery(exercise, s);
					solutionDao.insertNewInstance(solution);
				} else if (type.toLowerCase().equals("insert")) {
					exercise = new Exercise(exgroupInsert, null, q, type, Byte.parseByte(c), new Date(), null, null,
							null, null, null);
					exerciseDao.insertNewInstance(exercise);
					solution = new SolutionQuery(exercise, s);
					solutionDao.insertNewInstance(solution);
		...
		//		createScriptFile = "scenario_01.sql";
		//		scenario = new Scenario("SWT2015", new Date(), null, null, "<h1>foo</h1><p>text...text...text</p>",
