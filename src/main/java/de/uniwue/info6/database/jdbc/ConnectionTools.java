...
public class ConnectionTools extends Thread {
	...
	private void initStatsLog() {
		...
		this.executerStatsFile = SCRIPT_PATH + File.separator + RESOURCE_PATH + File.separator + SUB_DIR
				+ File.separator + STATS_DIR + File.separator + "executer_log_" + date + "_00.csv";
	...
	private synchronized void logExecuterStats(String string) {
		if (string != null && !string.replaceAll("\\s", "").isEmpty()) {
			try (PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter(executerStatsFile, true)))) {
				out.println(string);
			} catch (Exception e) {
	...
	private synchronized void logSessionStats(String string) {
		if (string != null && !string.replaceAll("\\s", "").isEmpty()) {
			try (PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter(sessionStatsFile, true)))) {
				out.println(string);
			} catch (Exception e) {
