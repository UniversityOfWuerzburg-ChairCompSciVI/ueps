 ...
 *
 * <pre>
 * EquivalenceLock <String> equivalenceLock = new EquivalenceLock<String>();
...
public class EquivalenceLock<T> {
  private static final Logger LOG = Logger.getLogger(EquivalenceLock.class
                                    .getName());
  ...
  public void lock(final T ticket) throws InterruptedException {
    final String threadName = DEBUG ? Thread.currentThread().getName()
                              : null;
    if (DEBUG)
  ...
  public void release(final T ticket) {
    synchronized (slots) {
      if (DEBUG)
        LOG.finest(Thread.currentThread().getName() + " tossing "
                   + ticket);
      slots.remove(ticket);
      slots.notifyAll();
    }
