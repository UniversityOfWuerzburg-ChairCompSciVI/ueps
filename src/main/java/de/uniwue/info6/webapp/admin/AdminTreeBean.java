	...
	public String getNodeName() {
		if (exerciseNode != null) {
			if (exerciseNode.isScenario()) {
				...
				return System.getProperty("EXERCISE_GROUP") + " ";
			} else if (exerciseNode.isExercise()) {
				return System.getProperty("EXERCISE") + " ";
			}
		}
		return "---" + " ";
	}
	...
	public void setNodeToCopy(boolean move) {
		...
		if (exerciseNode != null) {
			...
			if (exerciseNode.isExercise()) {
				...
				context.addMessage(
						null,
						new FacesMessage(System.getProperty("ADMINTREE.CLIPBOARD") + ":", System
								.getProperty("EXERCISE_GROUP") + ": [" + copiedExercise.getId() + "]"));
			} else if (exerciseNode.isExerciseGroup()) {
