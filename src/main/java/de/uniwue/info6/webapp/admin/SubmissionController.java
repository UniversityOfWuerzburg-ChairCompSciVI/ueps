...
public class SubmissionController implements Serializable {
	...
	private static final String EMPTY = "[" + System.getProperty("ASSERTION.EMPTY_FIELD") + "]";
	private static final String EMPTY_SC = System.getProperty("ASSERTION.NO_SCENARIO");
	private static final String EMPTY_GR = System.getProperty("ASSERTION.NO_GROUP");
	private static final String EMPTY_EX = System.getProperty("ASSERTION.NO_EX");
	private static final String EMPTY_US = System.getProperty("RIGHTS.USER_NOT_FOUND");
	...
	public void reloadTree() {
		...
		if (countEntries > 0) {
			if (countEntries == 1) {
				msg = countEntries + " " + System.getProperty("ASSERTION.FILTER3");
			} else {
				msg = countEntries + " " + System.getProperty("ASSERTION.FILTER2");
			}
			sev = FacesMessage.SEVERITY_INFO;
		}

		FacesMessage message = new FacesMessage(sev, System.getProperty("ASSERTION.FILTER1"), msg);
	...
	private void initTree() {
		try {
			...
			if (filter) {
				TreeNode node0 = new DefaultTreeNode(new SubmissionRow(System.getProperty("SCENARIO")), root);
									...
									for (Exercise ex : exercises) {
										...
										if (exerciseFilter) {
											...
											if (System.currentTimeMillis() - starttime > maxTime) {
												errorText = System.getProperty("ASSERTION.PERF_ERROR");
												break;
											}
