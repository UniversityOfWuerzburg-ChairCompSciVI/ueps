...
public class SubmissionEditor implements Serializable {
  ...
  public void init() {
    ...
    try {
      ...
      if (!requestParams.isEmpty() && requestParams.containsKey(EXERCISE_PARAM)
          && requestParams.containsKey(SUBMISSION_PARAM) && requestParams.containsKey(RESULT_PARAM)) {
        ...
        if (exercise != null && userEntry != null && userResult != null) {
          ...
          if (solutions != null) {
            ...
            for (SolutionQuery tmp : solutions) {
              ...
              int curDist = LevenshteinDistance.computeLevenshteinDistance(tmp.getQuery().toLowerCase(),
                  userEntry.getUserQuery().toLowerCase());
    ...
    if (userResult != null) {
      ...
      if (exercise != null) {
        if (userResult.getCredits() > exercise.getCredits()) {
          ...
          message = Cfg.inst().getProp(DEF_LANGUAGE, "ASSERTION.MAX_CREDITS") + ": " + exercise.getCredits();
        }
      }
      if (message == null) {
  ...
  public String getCommentText() {
    String comment = "---";
    if (userResult != null && newComment != null && !newComment.trim().isEmpty()) {
      return newComment;
    }
    return comment;
  }
  ...
  private FacesMessage syntaxCheck() {
    ...
    try {
      ...
      if (qu != null && !qu.trim().isEmpty()) {
        ...
        if (error != null) {
          message += Cfg.inst().getProp(DEF_LANGUAGE, "ASSERTION.SYNTAX_ERROR") + ": " + queryText + "<br/>" + error
              + "<br/>";
        }
      }
      if (!message.isEmpty()) {
  ...
  public void addAsSolution(ActionEvent actionEvent) {
    ...
    SolutionQuery newQuery = new SolutionQuery(exercise, userEntry.getUserQuery());
    ...
    try {
      ...
      if (msg != null && msg.getSummary() != null && !msg.getSummary().isEmpty()) {
        message = msg.getSummary();
        sev = FacesMessage.SEVERITY_ERROR;
      } else {
        newQuery.setStatus((byte) 2); //0 initial, 1 user used, 2 user unused
        ...
        if (saved) {
          ...
          sev = FacesMessage.SEVERITY_INFO;
          message = Cfg.inst().getProp(DEF_LANGUAGE, "SAVED.SOLUTION_SUCCESS") + ".";

        } else {

          sev = FacesMessage.SEVERITY_ERROR;
          message = Cfg.inst().getProp(DEF_LANGUAGE, "SAVED.SOLUTION_FAIL") + ".";
        ...
        if (saved && !saved) {
          ...
          List<UserEntry> userEntries = userEntryDao.getLastUserEntryForAllUsers(exercise);

          if (userEntries != null) {

            List<SolutionQuery> solutionsOrig = exerciseDao.getSolutions(exercise);
            ...
            for (UserEntry userEntry : userEntries) {
              ...
              if (userResult != null && userResult.getUser() == null) {
                ...
                if (userEntrySuccess) {
                  userResult.setUser(user);
                  userResult.setCredits(exercise.getCredits());
                  userResult.setLastModified(new Date());
        ...
        newQuery.setStatus((byte) 1); //0 initial, 1 user used, 2 user unused
