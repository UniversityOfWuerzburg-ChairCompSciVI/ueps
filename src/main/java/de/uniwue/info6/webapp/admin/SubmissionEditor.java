		...
		if (userResult != null) {
			...
			if (exercise != null) {
				if (userResult.getCredits() > exercise.getCredits()) {
					...
					message = System.getProperty("ASSERTION.MAX_CREDITS") + ": " + exercise.getCredits();
				}
			}
			if (message == null) {
	...
	private FacesMessage syntaxCheck() {
		...
		try {
			...
			if (qu != null && !qu.trim().isEmpty()) {
				...
				if (error != null) {
					message += System.getProperty("ASSERTION.SYNTAX_ERROR") + ": " + queryText + "<br/>" + error
							+ "<br/>";
				}
			}
			if (!message.isEmpty()) {
		...
		try {
			...
			if (msg != null && msg.getSummary() != null && !msg.getSummary().isEmpty()) {
				...
				if (saved) {
					...
					sev = FacesMessage.SEVERITY_INFO;
					message = System.getProperty("SAVED.SOLUTION_SUCCESS") + ".";

				} else {

					sev = FacesMessage.SEVERITY_ERROR;
					message = System.getProperty("SAVED.SOLUTION_FAIL") + ".";
