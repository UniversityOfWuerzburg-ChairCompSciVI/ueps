...
public class ExGroupTableBean implements Serializable {
	...
	private String getTimeDifference(Date start, Date end) {
		...
		return (" - [Noch "
				+ String.format("%d Tag%s, %d Stunde%s, %d Minute%s", diff[0], diff[0] > 1 ? "e" : "", diff[1],
						diff[1] > 1 ? "n" : "", diff[2], diff[2] > 1 ? "n" : "") + "]")
				.replace("0 Tag, ", "").replace("0 Stunde, ", "").replace(", 0 Minute", "");
	}
	...
	public String getGroupName(ExerciseGroup group) {
		if (group != null && group.getIsRated() != null) {
			return (group.getIsRated() ? System.getProperty("AC.EX_RA") : System.getProperty("AC.EX")) + "-&#8195;"
					+ StringTools.trimToLengthIndicator(group.getName(), 45) + resultFeedback(group);
		}
		return "ERROR";
	}
	...
	public int selectionSize(ExerciseGroup group) {
		if (selectedExercises != null && lastExerciseGroup != null && group != null && lastExerciseGroup.equals(group)) {
